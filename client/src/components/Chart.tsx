...
function Chart() {
  ...
  // 3개월 가격 데이터
  const { isLoading, data: stockChart } = useQuery<IDatePrevStockInfo[]>(
    "stockChart",
    () => getChartData(id || ""),
    {
      retry: true,
  ...
  const {
    ...
    error,
    refetch,
  } = useQuery<IDatePrevStockInfo[]>("todayChart", () => getTodayChartData(id || ""), {
    refetchInterval: stockTime() ? 5000 : false,
    retry: true,
  });
